...
void GPIO_configurePortClock(uint8_t portNum)
{
	...
	uint32_t PORT_ClockControl[NUM_OF_PORTS] = {SYSCTL_RCGC2_GPIOA, SYSCTL_RCGC2_GPIOB, SYSCTL_RCGC2_GPIOC, SYSCTL_RCGC2_GPIOD, SYSCTL_RCGC2_GPIOE, SYSCTL_RCGC2_GPIOF};
...
void GPIO_init(GPIO_ConfigurationType *config_ptr)
{
	...
	if ((config_ptr->port_num == PORTC_ID) && (config_ptr->pin_num < 4))
	{
...
void GPIO_writePin(uint8_t portNum, uint8_t pinNum, uint8_t value)
{
	...
	else
	{
		...
		volatile uint32_t *DATA_Registers[] = {&GPIO_PORTA_DATA_R,
											   &GPIO_PORTB_DATA_R,
											   &GPIO_PORTC_DATA_R,
											   &GPIO_PORTD_DATA_R,
											   &GPIO_PORTE_DATA_R,
		...
		WRITE_BIT(*DATA_Registers[portNum], pinNum, value);
	}
}
/*
 * Description :
...
uint8_t GPIO_readPin(uint8_t portNum, uint8_t pinNum)
{
	...
	else
	{
		...
		GET_BIT(*DATA_Registers[portNum], pinNum);
	}
}
